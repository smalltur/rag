...
class PipelineConfig:
    def __init__(self, root_path: Path, subset_name: str = "subset.csv", questions_file_name: str = "questions.json", pdf_reports_dir_name: str = "pdf_reports", serialized: bool = False, config_suffix: str = ""):
        ...
        # The bundled datasets store prebuilt indexes directly under the dataset root.
        legacy_vector_db_dir = root_path / "vector_dbs"
        legacy_documents_dir = root_path / "chunked_reports"
        if not self.databases_path.exists() and legacy_vector_db_dir.exists() and legacy_documents_dir.exists():
            self.vector_db_dir = legacy_vector_db_dir
            self.documents_dir = legacy_documents_dir
            self.bm25_db_path = root_path / "bm25_dbs"
...
class RunConfig:
    ...
    answering_model: str = "deepseek-chat"
    config_suffix: str = ""
...
base_config = RunConfig(
    ...
    pipeline_details="Custom pdf parsing + vDB + Router + SO CoT; llm = DeepSeek-V3",
    config_suffix="_base"
)
...
parent_document_retrieval_config = RunConfig(
    ...
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + SO CoT; llm = DeepSeek-V3",
...
max_config = RunConfig(
    ...
    pipeline_details="Custom pdf parsing + table serialization + vDB + Router + Parent Document Retrieval + reranking + SO CoT; llm = DeepSeek-V3",
    answering_model="deepseek-chat",
    config_suffix="_max"
)
...
max_no_ser_tab_config = RunConfig(
    ...
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + reranking + SO CoT; llm = DeepSeek-V3",
    answering_model="deepseek-chat",
...
max_st_o3m_config = RunConfig(
    ...
    pipeline_details="Custom pdf parsing + tables serialization + Router + vDB + Parent Document Retrieval + reranking + SO CoT; llm = DeepSeek-V3",
    answering_model="deepseek-chat",
...
max_nst_o3m_config_big_context = RunConfig(
    ...
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + reranking + SO CoT; llm = DeepSeek-V3; top_n = 14; topn for rerank = 36",
    answering_model="deepseek-chat",
