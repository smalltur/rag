...
class VectorRetriever:
    ...
    def _set_up_llm(self):
        ...
        if not embedding_base_url or not embedding_api_key:
            raise ValueError(
                "DeepSeek does not provide an Embeddings API. Set EMBEDDING_BASE_URL and "
                "EMBEDDING_API_KEY to the service used when creating the vector indexes."
            )
        llm = OpenAI(
    ...
    def _load_dbs(self):
        ...
        for document_path in all_documents_paths:
            ...
            try:
                index_bytes = np.fromfile(vector_db_files[stem], dtype=np.uint8)
                vector_db = faiss.deserialize_index(index_bytes)
            except Exception as e:
