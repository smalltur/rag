...
class BaseOpenaiProcessor:
    ...
    def send_message(
        ...
        elif "deepseek" in (os.getenv("OPENAI_BASE_URL") or "").lower():
            params["response_format"] = {"type": "json_object"}
            completion = self.llm.chat.completions.create(**params)
            parsed_dict = json.loads(repair_json(completion.choices[0].message.content))
            content = response_format.model_validate(parsed_dict).model_dump()
...
class APIProcessor:
    def __init__(self, provider: Literal["deepseek", "openai", "ibm", "gemini"] ="deepseek"):
    ...
    def _build_rag_context_prompts(self, schema):
        ...
        use_schema_prompt = self.provider in {"deepseek", "ibm", "gemini"}
    ...
    def get_rephrased_questions(self, original_question: str, companies: List[str]) -> Dict[str, str]:
        ...
        system_prompt = (
            ...
            else prompts.RephrasedQuestionsPrompt.system_prompt
        )
        answer_dict = self.processor.send_message(
            system_content=system_prompt,
...
class AsyncOpenaiProcessor:
    ...
    async def process_structured_ouputs_requests(
        ...
        if request_url is None:
            request_url = f"{os.getenv('OPENAI_BASE_URL', 'https://api.deepseek.com').rstrip('/')}/chat/completions"
